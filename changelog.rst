...
- [core] Allow ``Observer.schedule(None, path)`` (and :data:`watchdog.events.noop_event_handler`) to register a watch without an initial handler; use :meth:`watchdog.observers.api.BaseObserver.add_handler_for_watch` to attach handlers afterward.
