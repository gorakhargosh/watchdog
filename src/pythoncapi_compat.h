...
#if PY_VERSION_HEX < 0x030C00A2 && !defined(PYPY_VERSION)
static inline PyObject *PyFrame_GetVarString(PyFrameObject *frame,
                                             const char *name) {
...
static inline int PyModule_AddObjectRef(PyObject *module, const char *name,
                                        PyObject *value) {
  int res;
...
static inline int PyObject_GetOptionalAttr(PyObject *obj, PyObject *attr_name,
                                           PyObject **result) {
...
static inline int PyObject_GetOptionalAttrString(PyObject *obj,
...
#if PY_VERSION_HEX < 0x030D00A1
static inline int PyMapping_GetOptionalItem(PyObject *obj, PyObject *key,
                                            PyObject **result) {
...
static inline int PyMapping_GetOptionalItemString(PyObject *obj,
                                                  const char *key,
                                                  PyObject **result) {
  PyObject *key_obj;
  int rc;
...
static inline int PyMapping_HasKeyStringWithError(PyObject *obj,
                                                  const char *key) {
  PyObject *res;
...
static inline int PyObject_HasAttrStringWithError(PyObject *obj,
                                                  const char *attr) {
  PyObject *res;
...
// to Python 3.13.0a1
#if PY_VERSION_HEX < 0x030D00A1
static inline int PyDict_GetItemRef(PyObject *mp, PyObject *key,
                                    PyObject **result) {
#if PY_VERSION_HEX >= 0x03000000
...
static inline int PyDict_GetItemStringRef(PyObject *mp, const char *key,
                                          PyObject **result) {
  int res;
#if PY_VERSION_HEX >= 0x03000000
...
#if PY_VERSION_HEX < 0x030D00A1
static inline int PyModule_Add(PyObject *mod, const char *name,
                               PyObject *value) {
...
#if PY_VERSION_HEX < 0x030D00A1
static inline int PyObject_VisitManagedDict(PyObject *obj, visitproc visit,
                                            void *arg) {
...
// to Python 3.13.0a1
#if PY_VERSION_HEX < 0x030D00A1
static inline int PyUnicode_EqualToUTF8AndSize(PyObject *unicode,
                                               const char *str,
                                               Py_ssize_t str_len) {
  Py_ssize_t len;
  const void *utf8;
...
static inline int PyDict_PopString(PyObject *dict, const char *key,
                                   PyObject **result) {
  PyObject *key_obj = PyUnicode_FromString(key);
  if (key_obj == NULL) {
    if (result != NULL) {
...
// gh-114329 added PyList_GetItemRef() to Python 3.13.0a4
#if PY_VERSION_HEX < 0x030D00A4
static inline int PyDict_SetDefaultRef(PyObject *d, PyObject *key,
...
static inline int PyUnicodeWriter_WriteChar(PyUnicodeWriter *writer,
                                            Py_UCS4 ch) {
  if (ch > 0x10ffff) {
...
static inline int PyUnicodeWriter_WriteStr(PyUnicodeWriter *writer,
                                           PyObject *obj) {
...
static inline int PyUnicodeWriter_WriteRepr(PyUnicodeWriter *writer,
                                            PyObject *obj) {
  if (obj == NULL) {
...
static inline int PyUnicodeWriter_WriteUTF8(PyUnicodeWriter *writer,
...
static inline int PyUnicodeWriter_WriteASCII(PyUnicodeWriter *writer,
                                             const char *str, Py_ssize_t size) {
  if (size < 0) {
    size = (Py_ssize_t)strlen(str);
  }

  return _PyUnicodeWriter_WriteASCIIString((_PyUnicodeWriter *)writer, str,
                                           size);
}

static inline int PyUnicodeWriter_WriteWideChar(PyUnicodeWriter *writer,
                                                const wchar_t *str,
                                                Py_ssize_t size) {
  if (size < 0) {
...
static inline int PyUnicodeWriter_WriteSubstring(PyUnicodeWriter *writer,
                                                 PyObject *str,
                                                 Py_ssize_t start,
                                                 Py_ssize_t end) {
  if (!PyUnicode_Check(str)) {
    PyErr_Format(PyExc_TypeError, "expect str, not %s", Py_TYPE(str)->tp_name);
    return -1;
  }
  ...
  return _PyUnicodeWriter_WriteSubstring((_PyUnicodeWriter *)writer, str, start,
                                         end);
}
...
static inline void _PyLong_SetSignAndDigitCount(PyLongObject *op, int sign,
                                                Py_ssize_t size) {
...
static inline int Py_fclose(FILE *file) { return fclose(file); }
#endif
...
#define PYTHONCAPI_COMPAT_SPEC(MEMBER, TYPE, sys_attr)                         \
  { #MEMBER, offsetof(PyConfig, MEMBER), _PyConfig_MEMBER_##TYPE, sys_attr }
...
static inline PyObject *PySys_GetAttrString(const char *name) {
#if PY_VERSION_HEX >= 0x03000000
...
static inline PyObject *PySys_GetAttr(PyObject *name) {
#if PY_VERSION_HEX >= 0x03000000
...
static inline int PySys_GetOptionalAttrString(const char *name,
                                              PyObject **value) {
#if PY_VERSION_HEX >= 0x03000000
...
static inline int PySys_GetOptionalAttr(PyObject *name, PyObject **value) {
#if PY_VERSION_HEX >= 0x03000000
  const char *name_str = PyUnicode_AsUTF8(name);
#else
  const char *name_str = PyString_AsString(name);
...
static inline Py_ssize_t _PyBytesWriter_GetAllocated(PyBytesWriter *writer) {
  if (writer->obj == NULL) {
...
static inline int _PyBytesWriter_Resize_impl(PyBytesWriter *writer,
                                             Py_ssize_t size, int resize) {
  ...
  if (writer->obj != NULL) {
    ...
    if (resize) {
      ...
      memcpy(PyBytes_AS_STRING(writer->obj), writer->small_buffer,
...
static inline void *PyBytesWriter_GetData(PyBytesWriter *writer) {
  if (writer->obj == NULL) {
...
static inline Py_ssize_t PyBytesWriter_GetSize(PyBytesWriter *writer) {
  return writer->size;
}

static inline void PyBytesWriter_Discard(PyBytesWriter *writer) {
  if (writer == NULL) {
    return;
  }
...
static inline PyBytesWriter *PyBytesWriter_Create(Py_ssize_t size) {
  if (size < 0) {
...
static inline PyObject *PyBytesWriter_FinishWithSize(PyBytesWriter *writer,
                                                     Py_ssize_t size) {
  PyObject *result;
  if (size == 0) {
...
static inline PyObject *PyBytesWriter_Finish(PyBytesWriter *writer) {
  return PyBytesWriter_FinishWithSize(writer, writer->size);
}

static inline PyObject *PyBytesWriter_FinishWithPointer(PyBytesWriter *writer,
                                                        void *buf) {
...
static inline int PyBytesWriter_Resize(PyBytesWriter *writer, Py_ssize_t size) {
  if (size < 0) {
    PyErr_SetString(PyExc_ValueError, "size must be >= 0");
    return -1;
  }
...
static inline int PyBytesWriter_Grow(PyBytesWriter *writer, Py_ssize_t size) {
...
static inline void *PyBytesWriter_GrowAndUpdatePointer(PyBytesWriter *writer,
                                                       Py_ssize_t size,
                                                       void *buf) {
...
static inline int PyBytesWriter_WriteBytes(PyBytesWriter *writer,
...
static inline int PyBytesWriter_Format(PyBytesWriter *writer,
                                       const char *format, ...)
...
static inline int PyBytesWriter_Format(PyBytesWriter *writer,
  ...
  int res = PyBytesWriter_WriteBytes(writer, PyBytes_AS_STRING(str),
...
#if PY_VERSION_HEX < 0x030F00A1
static inline PyObject *PyTuple_FromArray(PyObject *const *array,
                                          Py_ssize_t size) {
...
#if PY_VERSION_HEX < 0x030F00A1
static inline Py_hash_t PyUnstable_Unicode_GET_CACHED_HASH(PyObject *op) {
#ifdef PYPY_VERSION
  (void)op; // unused argument
  return -1;
...
// implemented for 3.14 using _Py_SetImmortal() and uses private macros on 3.13.
#if 0x030E0000 <= PY_VERSION_HEX
PyAPI_FUNC(void) _Py_SetImmortal(PyObject *op);
#endif

static inline int PyUnstable_SetImmortal(PyObject *op) {
  assert(op != NULL);
